...
MakeHillClimber=setRefClass(
  ...
  methods=list(
    ...
    # p2min p2max
    # p3min p3max
    initialize=function(evalf, step_s, seek_max,bounds){
      eval_function <<- evalf
      step_size     <<- step_s
    ...
    hill_climb=function(){
      adjacent=foreach(d=1:length(coords), .combine=rbind) %do% {
        rbind(replace(coords,d,coords[d]+step_size),
              replace(coords,d,coords[d]-step_size)
        )
      }
      vals=apply(adjacent,1,eval_function) # remember to grab the log from this too
      
      if(seek_maxima){
        coords=adjacent[which.max(vals),]
      } else {
        coords=adjacent[which.min(vals),]
      }
